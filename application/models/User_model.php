...

{
	...
	public  function save_roles_name($d){
		$this->db->insert('role',$d);
		return $this->db->insert_id();
	}
	public  function get_admin_roles_list(){
		$this->db->select('*')->from('role');
		$this->db->where('status !=',2);
		return $this->db->get()->result_array();
	}
	
	public  function check_normal_role($n){
		$this->db->select('*')->from('role');
		$this->db->where('name',$n);
		$this->db->where('status !=',2);
		return $this->db->get()->row_array();	
	}	
	public  function update_rolesname($id,$d){
		$this->db->where('r_id',$id);
		return $this->db->update('role',$d);
	}
