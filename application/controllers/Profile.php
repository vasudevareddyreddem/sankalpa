...
class Profile extends sidebar {
	...
	{	
		...
		{
			$l_data=$this->session->userdata('hms_details');
			$data['u_d']=$this->Admin_model->get_emp_full_details($l_data['a_id']);
	...
	public function edit()
	{
		if($this->session->userdata('hms_details'))
		{
			...
			if($l_data['role_id']==2){
				...
				$data['emp_d']=$this->Admin_model->get_emp_full_details($l_data['a_id']);
				//echo '<pre>';print_r($data);exit;
				$this->load->view('employee/emp-edit-profile',$data);
			}else{
